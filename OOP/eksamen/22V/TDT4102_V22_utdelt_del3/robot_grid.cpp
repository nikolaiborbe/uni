...
void RobotGrid::draw_grid_lines()
{
    ...
    for (int y = y_pos; y <= h_size + y_pos; y += cell_height) {
        window.draw_line(
            TDT4102::Point{x_pos, y}, 
            TDT4102::Point{x_pos + w_size, y}
        );
    }

    for (int x = x_pos; x <= w_size + x_pos; x += cell_width){
        window.draw_line(
            TDT4102::Point{x, y_pos}, 
            TDT4102::Point{x, y_pos + h_size}
        );
    }
...
Point RobotGrid::get_grid_cell_center_coord(int x, int y) const
{
  ...
  // already there and replace it with your own.
    int x_edge = x_pos + (cell_width * x);
    int y_edge = y_pos + (cell_height * y);

    int center_x = x_edge + cell_width/2;
    int center_y = y_edge + cell_height/2;
...
void RobotGrid::make_robot(string name, Point pos, Color color)
{
  ...
  // already there and replace it with your own.
    robots[name] = std::make_unique<Robot>(name,pos,color);
...
void RobotGrid::draw_robots()
{
  ...
  // already there and replace it with your own.
    ...
    for (const auto& [name, ptr_robot] : robots) {
        window.draw_circle(
            get_grid_cell_center_coord(ptr_robot->pos),
            std::min(cell_width, cell_height)/2,
            ptr_robot->color
        );

        auto [x,y] = get_grid_cell_edge_coord(ptr_robot->pos);
        window.draw_text(
            {x,y},
            ptr_robot->name
        );
...
void RobotGrid::rename_robot(string name, string new_name)
{
  ...
  // already there and replace it with your own.
    ...
    robots[new_name] = make_unique<Robot>(*robots[name]);
    robots.erase(name);
...
void RobotGrid::check_coord_bounds(Point p) const
{
  ...
  // already there and replace it with your own.
    ...
    if (!(0 <= p.x && p.x < cols)) 
        throw std::runtime_error("x coordinate is outside the grid");

    if (!(0 <= p.y && p.y <rows))
        throw std::runtime_error("y coordinate is outside the grid");
...
void RobotGrid::check_name_available(string name) const
{
  ...
  // already there and replace it with your own.
    ...
    if (robots.contains(name)) {
        throw std::runtime_error("Robot name in use.");
    }
...
void RobotGrid::check_name_exists(string name) const
{
  ...
  // already there and replace it with your own.
    if (!(robots.contains(name))) {
        throw std::runtime_error("Name in use");
    }
  (void)name;
...
void RobotGrid::check_coord_empty(Point p, string name, bool is_moving) const
{
  ...
  // already there and replace it with your own.
    ...
    if (is_moving) {
        for (const auto& [i_name, i_robot] : robots){
            bool same_robot = i_name == name;
            bool same_position = i_robot->pos.x == x && i_robot->pos.y == y;

            if (!same_robot && same_position) {
                throw std::runtime_error("Can not move robot to this point");
            }
        }
    } else if (!is_moving) {
        for (const auto& [i_name, i_robot] : robots){
            ...
            if (!same_robot && same_position) {
                throw std::runtime_error("Can not make robot to this point");
            }
        }
    }
